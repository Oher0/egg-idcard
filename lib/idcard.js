...
class Idcard {
  ...
  random() {
    const year = parseInt(Math.random() * 51, 10) + 49;
    let month;
    let day;
    let area;
    let flag = true;
    while (flag) {
      const provinceCode = parseInt(Math.random() * 72, 10) + 11 + '';
      let cityCode = parseInt(Math.random() * 30, 10) + 1;
      let districtCode = parseInt(Math.random() * 30, 10) + 1;
      cityCode < 10 ? cityCode = '0' + '' + cityCode : cityCode += '';
      districtCode < 10 ? districtCode = '0' + '' + districtCode : districtCode += '';
      area = provinceCode + cityCode + districtCode;
      if (district[area]) flag = false;
    }

    const rand = parseInt(Math.random() * 898, 10) + 100 + '';
    month = parseInt(Math.random() * 12, 10) + 1;
    month < 10 ? month = '0' + '' + month : month += '';
    day = parseInt(Math.random() * 28, 10) + 1;
    day < 10 ? day = '0' + '' + day : day += '';
    const idno = area + year + month + day + rand;
    assert(idno.length === 15, idno);
    return this.repair(idno);
  }
}
